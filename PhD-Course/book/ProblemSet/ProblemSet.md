...
This page groups the problem sets by course and by theme. Start from the course-level section, then drill into the individual problem.
...
- 考点：资产定价、无套利、一价定律、市场完备性、状态价格、Arrow 证券、风险中性测度、SDF、套利策略

跨课链接：[State prices, SDF, and complete markets](../Cross-Course/bridges/State_Prices_SDF_Complete_Markets.md)
...
- The number of linearly independent assets $N$ is the rank of $D$:
  见 [矩阵秩 rank](cards/矩阵秩%20rank.md)。（行/列变换后非零行/列的数量就是秩），所以$\operatorname{rank}(D)=2.$
- **Conclusion**:
...
#### 2.1 Arrow Security Price 的数学推导
...
这就是 CAPM. 又因为 $R_m$ 自己满足 $\beta_m=1$，并且它是同均值 risky portfolio 里方差最小的那个，所以 $R_m$ 必在 mean-variance efficient frontier 上。更完整的等价证明见 [CAPM and Mean-Variance Efficiency](Asset Pricing/Theoretical AP/01_Single_Period_Models#^prop-3-2-4-mve)。
...
> 1. 对任意信息集 $\mathcal I$，写出 [CARA-Normal](Asset Pricing/Theoretical AP/cards/CARA-Normal framework - 均值方差等价与最优需求) 下的 certainty equivalent，并求最优持仓 $X^*(\mathcal I)$。
...
在信息集 $\mathcal I$ 下，[CARA-Normal](Asset Pricing/Theoretical AP/cards/CARA-Normal framework - 均值方差等价与最优需求) 等价于最大化
...
关联：[Asset Pricing/Theoretical AP/cards/part2/状态价格密度、EMM 与鞅定价](Asset Pricing/Theoretical AP/cards/part2/状态价格密度、EMM 与鞅定价)；[dynamic AP: Arbitrage, SPD, and EMM](Asset Pricing/Theoretical AP/02_Dynamic_Asset_Pricing#4.3 Arbitrage, SPD, and EMM)
...
股票是 aggregate Lucas tree，dividend 是 $\delta_t$（见 [Asset Pricing/Theoretical AP/cards/part2/普通股票设定与 Lucas tree 股票设定](Asset Pricing/Theoretical AP/cards/part2/普通股票设定与 Lucas tree 股票设定)）。股票价格为：

:::{admonition} Note
+ 复习卡片：Lucas tree stock pricing
关联：[Asset Pricing/Theoretical AP/cards/part2/普通股票设定与 Lucas tree 股票设定](Asset Pricing/Theoretical AP/cards/part2/普通股票设定与 Lucas tree 股票设定)；[Asset Pricing/Theoretical AP/cards/part2/Lucas tree model 与代表性消费者均衡](Asset Pricing/Theoretical AP/cards/part2/Lucas tree model 与代表性消费者均衡)；[dynamic AP: Lucas Tree Equilibrium](Asset Pricing/Theoretical AP/02_Dynamic_Asset_Pricing#6.2 Lucas Tree Equilibrium)
...
:::{admonition} Note
+ 复习卡片：动态预算约束
关联：[Asset Pricing/Theoretical AP/cards/part2/连续时间组合选择问题](Asset Pricing/Theoretical AP/cards/part2/连续时间组合选择问题)；[dynamic AP: Portfolio Choice](Asset Pricing/Theoretical AP/02_Dynamic_Asset_Pricing#5.1 Portfolio Choice in Continuous Time)
...
关联：[Asset Pricing/Theoretical AP/cards/part2/普通股票设定与 Lucas tree 股票设定](Asset Pricing/Theoretical AP/cards/part2/普通股票设定与 Lucas tree 股票设定)；[Asset Pricing/Theoretical AP/cards/part2/连续时间组合选择问题](Asset Pricing/Theoretical AP/cards/part2/连续时间组合选择问题)
...
关联：[ProblemSet/cards/Radon-Nikodym density 与 Girsanov 复习](ProblemSet/cards/Radon-Nikodym density 与 Girsanov 复习)；[ProblemSet/cards/OU-Vasicek 过程复习](ProblemSet/cards/OU-Vasicek 过程复习)
...
细节见 [ProblemSet/cards/Radon-Nikodym density 与 Girsanov 复习](ProblemSet/cards/Radon-Nikodym density 与 Girsanov 复习).
...
+
**复习卡片：OU/Vasicek 过程**
这里用到 OU 过程的显式解、条件均值和方差；细节见 [ProblemSet/cards/OU-Vasicek 过程复习](ProblemSet/cards/OU-Vasicek 过程复习).
...
> 
:::{admonition} （a）
Suppose that news-watchers have utility function
...
见 [CARA-Normal framework](Asset Pricing/Theoretical AP/cards/CARA-Normal framework - 均值方差等价与最优需求)。核心是：
$$
\begin{aligned}
W\mid \mathcal I
...
$$
所以目标函数变成（见 [certainty equivalent 推导](Asset Pricing/Theoretical AP/cards/CARA-Normal framework - 均值方差等价与最优需求)）：
$$
\begin{aligned}
CE_{it}(x_{it})
&=
...
>
:::{admonition} （a）
Assuming that $W$ is normally distributed, show that the insider’s optimization problem is equivalent to maximizing
...
回顾卡片：[One-period Kyle 模型复习](ProblemSet/cards/One-period Kyle 模型复习)
...
### Q 1.2(a)
![[attachment/Pasted image 20260428115441.png]]
...
### Q 1.2(b,d)
![[attachment/Pasted image 20260428101237.png]]
...
![[attachment/Pasted image 20260428104131.png]]
...
![[attachment/Pasted image 20260428110613.png]]
...
![[attachment/Pasted image 20260428122408.png]]
...
![[attachment/Pasted image 20260428125930.png]]
...
### Q 6.1
![[attachment/Pasted image 20260428133349.png]]

:::{admonition} Note
+ Factor model
核心是用少数 common factors 解释 assets 的 expected returns。详细定义见 [general factor models](Asset Pricing/Theoretical AP/01_Single_Period_Models#^prop-3-2-6-factor-models)、[Single-Factor Model](Asset Pricing/Theoretical AP/01_Single_Period_Models#^def-3-2-7-single-factor)、[Multifactor Model](Asset Pricing/Theoretical AP/01_Single_Period_Models#^def-3-2-8-multifactor)。
...
Empirical AP 对应：[Linear SDF implies beta pricing](../Asset Pricing/Empirical AP/05_Cross_Section_Factor_Models#^lemma-linear-sdf-beta-pricing)；汇总卡片：[Beta-SDF Equivalence](../Asset Pricing/Empirical AP/cards/Beta_SDF_Equivalence)。
...
这正是 risk-free asset 存在时 frontier portfolio 的权重形式；推导见 [risk-free frontier weight](Asset Pricing/Theoretical AP/01_Single_Period_Models#^prop-3-1-risk-free-frontier-weight)：
...
![[attachment/Pasted image 20260428141951.png]]
...
![[attachment/Pasted image 20260428151130.png]]
...
![[attachment/Pasted image 20260428151432.png]]
...
![[attachment/Pasted image 20260428155824.png]]
...
![[attachment/Pasted image 20260428162212.png]]
...
>工具公式见 [正态条件方差](Asset Pricing/Theoretical AP/cards/正态条件方差) 与 [贝叶斯更新：先验、似然、后验](Asset Pricing/Theoretical AP/cards/贝叶斯更新 - 先验、信号、似然、后验)。本题只使用其中的 Gaussian / linear projection 后验方差更新。
...
![[attachment/Pasted image 20260428183959.png]]
...
![[attachment/Pasted image 20260428194818.png]]
...
![[attachment/Pasted image 20260428195122.png]]
...
![[attachment/Pasted image 20260428203158.png]]
...
![[attachment/Pasted image 20260428204347.png]]
...
![[attachment/Pasted image 20260428205902.png]]
...
![[attachment/Pasted image 20260428214804.png]]
...
![[attachment/Pasted image 20260428221909.png]]
...
### 11. Risk-Free Asset and Beginning/End-of-Period Consumption
...
### 5.3 No Tangency Portfolio When Risk-Free Rate Equals GMV Return
...
### 6.1 Beta Pricing Implies the Frontier Property

:::{admonition} Note
+ 合并位置
这题已经在 [Q 6.1](#Q 6.1) 推导：beta-pricing by $\widetilde R_*$ $\Rightarrow$ $\widetilde R_*\in$ mean-variance frontier。核心公式见 [Q 6.1 的 frontier 权重推导](#^qe-ps-q6-1-frontier-weight)。
...
### 6.2 No-Risk-Free Beta Pricing and Mean-Variance Frontier
...
### 6.2A Single-Factor Return and Mean-Variance Frontier
...
### 6.3 Borrowing and Lending at Different Rates
...
### 6.8 Limited Awareness and Positive Alpha
...
### 8.2 Marketed Consumption Processes
...
### 8.1 Marked Dividend Trees and SDFs
...
### 9.1 Log Utility with i.i.d. Returns
...
### 10.1 考场原题回忆：n Risky Assets + 0 Risk-Free Asset
...
:::{admonition} Note
+ 与 10.5 的关系
这题是 [10.5 no-risk-free mean-variance frontier](#^qe-ps-exam-no-risk-free-frontier) 的 CARA-normal 应用：10.5 解 fully invested frontier，10.1 在约束 $1'\phi=w_0$ 下直接解 investor 的 optimal risky dollar position。
...
\sigma_p^2
&=(\phi^*)'\Sigma\phi^* \\
&=\frac{1}{\alpha^2}\mu'\Sigma^{-1}\mu
+\frac{2(\alpha w_0-1'\Sigma^{-1}\mu)}{\alpha^2\,1'\Sigma^{-1}1}\,1'\Sigma^{-1}\mu
+\frac{(\alpha w_0-1'\Sigma^{-1}\mu)^2}{\alpha^2(1'\Sigma^{-1}1)^2}\,1'\Sigma^{-1}1 \\
&=\frac{\mu'\Sigma^{-1}\mu}{\alpha^2}
+\frac{(\alpha w_0-1'\Sigma^{-1}\mu)^2}{\alpha^2\,1'\Sigma^{-1}1}
\end{aligned}
...
### 10.2 考场原题回忆：CARA Endowment Economy and Budget Constraint
...
### 10.3 考场原题回忆：Two Countries, Two Goods, Exchange Rate, Log Utility
...
:::{admonition} Note
+ 合并位置
这题已合并到 [10.7 考场题重构：Two-Country Two-Good Pure-Exchange Economy](#^qe-ps-exam-two-country-two-good-pure-exchange)。10.7 保留完整的 optimization problems、goods clearing、FOC、allocation、exchange rate、interest rate、stock prices 和 symmetric case。
...
### 10.4 考场原题回忆：Bid-Ask Spread and Risk Aversion
...
### 10.5 考场题重构：No-Risk-Free Mean-Variance Frontier
...
^qe-ps-exam-no-risk-free-frontier

---

### 10.6 考场题重构：CARA Terminal Wealth, SPD, and Market Price of Risk
...
^qe-ps-exam-cara-terminal-spd

---

### 10.7 考场题重构：Two-Country Two-Good Pure-Exchange Economy
...
^qe-ps-exam-two-country-two-good-pure-exchange

---

### EMM and Market Completeness

Consider a continuous-time economy on a filtered probability space  
$(\Omega,\mathcal F,(\mathcal F_t)_{0\le t\le T},P)$, where $W_t$ is a Brownian motion. There are two traded assets: a money market account $B_t$ and a risky stock $S_t$. Their dynamics are

$$
\frac{dB_t}{B_t}=r,dt,\qquad B_0=1,

$$

$$
\frac{dS_t}{S_t}=\mu_t,dt+\sigma,dW_t,\qquad \sigma>0,

$$

where the stochastic expected return $\mu_t$ follows an Ornstein–Uhlenbeck process

$$
d\mu_t=\kappa(\theta-\mu_t),dt+\eta,dW_t.

$$

Assume all regularity conditions needed for Girsanov’s theorem hold.

 **(a) Define the market price of risk process.**

Show that the market price of risk is

$$
y_t=\frac{\mu_t-r}{\sigma}.

$$

 **(b) Construct an equivalent martingale measure (Q).**
...
\exp\left(  
-\int_0^t y_s,dW_s  
-\frac12\int_0^t y_s^2,ds  
\right).

$$

Assuming Novikov’s condition,
...
$$
W_t^Q=W_t+\int_0^t y_s,ds

$$

is a Brownian motion under $Q$.

 **(c) Write the asset dynamics under (Q).**
...
Thus the discounted stock price
...
so $\tilde S_t$ is a (Q)-martingale. Therefore $Q$ is an equivalent martingale measure.

Also derive the (Q)-dynamics of $\mu_t$:

$$
d\mu_t

\left[\kappa(\theta-\mu_t)-\eta y_t\right]dt+\eta,dW_t^Q.

$$

 **(d) State the no-arbitrage implication.**

Since an equivalent martingale measure exists, the model admits no arbitrage.

 **(e) Prove that the market is complete.**

Since the filtration is generated by one Brownian motion $W$, and there is one risky asset with nonzero volatility $\sigma S_t$, every square-integrable claim $X\in L^2(\mathcal F_T)$ can be replicated.

For any claim $X$, define its discounted price process

$$
M_t

E^Q\left[\frac{X}{B_T}\mid\mathcal F_t\right].

$$

By the martingale representation theorem, there exists a predictable process $\phi_t$ such that
...
choose the stock holding

$$
\Delta_t=\frac{\phi_t}{\sigma\tilde S_t}.

$$

Then the self-financing portfolio with stock holding $\Delta_t$ replicates $X$. Hence the market is complete.

 **（f） Write the risk-neutral pricing formula.**

For any contingent claim $X$ paid at time $T$,

$$
V_t

B_tE^Q\left[\frac{X}{B_T}\mid\mathcal F_t\right].

$$

If $r$ is constant, this becomes

$$
V_t=e^{-r(T-t)}E^Q[X\mid\mathcal F_t].
...
是题干给的 risk-free asset；

$$
\frac{dS_t}{S_t}=\mu_tdt+\sigma dW_t
...
就是 market price of risk；

“EMM, complete” 很像是小题要求：  
先写 (Q)-measure，再证明 discounted price 是 (Q)-martingale，最后证明 complete。

 **重要 caveat：如果原题里有两个 Brownian motions**
...
其中 $W_t$ 和 $Z_t$ 是两个独立 Brownian motions，那么市场一般**不完备**，因为只有一个 risky asset，却有两个风险源。此时 EMM 不唯一：

$$
\frac{dQ^\gamma}{dP}\Big|_{\mathcal F_T}

\exp\left(  
-\int_0^T y_t,dW_t  
-\int_0^T \gamma_t,dZ_t  
-\frac12\int_0^T(y_t^2+\gamma_t^2),dt  
\right),
...
由股票漂移决定，但 $\gamma_t$ 任意 admissible。EMM 不唯一 $\Rightarrow$ 市场不完备。

所以如果题目明确要求 “prove complete”，那最可能原题是**同一个 Brownian motion 驱动 $S_t$ 和 $\mu_t$**，或者 $d\mu_t$ 的 Brownian 风险完全由股票风险 span。

---

## Empirical Asset Pricing

題型一：Fama-MacBeth (1973) 兩步法與誤差修正（30分）

**Question:**  
Suppose you are testing a linear $K$-factor asset pricing model $E[R_i^e] = \beta_i \lambda$ using a cross-section of $N$ assets over $T$ periods.

1. **Methodology**: Clearly write down the specification for **Step 1 (Time-series)** and **Step 2 (Cross-sectional)** of the Fama-MacBeth procedure.
2. **Statistical Inference**: Explain why the standard OLS standard errors from Step 2 are biased downward (the "Errors-in-Variables" / EIV problem).
3. **Solution**: Provide the explicit mathematical formula for the **Shanken $1992$ correction** applied to the variance-covariance matrix of the risk premia ${\lambda }$.

 題型二：SDF 框架下的 GMM 估計（30分）

**Question:**  
Consider the Euler equation under the Stochastic Discount Factor $SDF$ framework:  
$E_{t}[m_{t+1}(1+R_{i,t+1})]=1$  
Assume a linear SDF model: $m_{t+1} = a - b \cdot f_{t+1}$, where $f_{t+1}$ is a single systematic risk factor.

1. **Moment Conditions**: Define the population moment conditions $g(\theta)$ used to estimate $\theta = (a, b)'$ via the **Generalized Method of Moments $GMM$** using a vector of excess returns $R_{t+1}^{e}$.
2. **Optimal Weighting Matrix**: Derive the formulation for the optimal weighting matrix $W^{*}$ under unconditional moments. Explain how to estimate this matrix empirically when the pricing errors exhibit heteroskedasticity and autocorrelation (e.g., Newey-West estimator).
3. **Specification Test**: Write down the **Hansen's $J$-statistic** formula. State its asymptotic distribution, degrees of freedom, and how to interpret a rejection of the null hypothesis in a QE context.

題型三：VIX / Variance Swap / option expansion（40分）

**Question:**  
Li 老师这道题最可能考的是：从 risk-neutral stock process 出发，用 Hull Technical Note 22 的思路，通过 option expansion 还原 conditional expected variance / implied variance。

1. **Stock Process to Realized Variance**: Assume under the risk-neutral measure

$$
\frac{dS_s}{S_s}=(r-q)ds+\sigma_s dW_s^{\mathbb Q},
\qquad \tau:=T-t.

$$

Use It\^o's lemma to show that

$$
\frac12\sigma_s^2ds=\frac{dS_s}{S_s}-d\ln S_s,

$$

and integrate over $[t,T]$ to express $\int_t^T \sigma_s^2ds$ in terms of $\int_t^T dS_s/S_s$ and $\ln(S_T/S_t)$.

2. **Conditional Expected Variance**: Take $E_t^{\mathbb Q}[\cdot]$ and show that

$$
E_t^{\mathbb Q}\!\left[\frac{1}{\tau}\int_t^T \sigma_s^2ds\right]
=
\frac{2}{\tau}\ln\frac{F_{t,T}}{S_t}
-\frac{2}{\tau}E_t^{\mathbb Q}\!\left[\ln\frac{S_T}{S_t}\right],

$$

where $F_{t,T}=S_t e^{(r-q)\tau}$.

3. **Option Expansion of the Log Contract**: For any $x>0$, use the Carr-Madan expansion with
...
=
\frac{S_T-x}{x}
-\int_0^x\frac{(K-S_T)^+}{K^2}dK
-\int_x^\infty\frac{(S_T-K)^+}{K^2}dK.

$$

4. **Convert Payoffs into Option Prices**: Define

$$
\left\{
\begin{aligned}
P_t(K,T)&=e^{-r\tau}E_t^{\mathbb Q}[(K-S_T)^+],\\
C_t(K,T)&=e^{-r\tau}E_t^{\mathbb Q}[(S_T-K)^+].
\end{aligned}
\right.
...
$$
E_t^{\mathbb Q}\!\left[\ln\frac{S_T}{x}\right]
=
\frac{F_{t,T}}{x}-1
-e^{r\tau}\left[
\int_0^x\frac{P_t(K,T)}{K^2}dK
+\int_x^\infty\frac{C_t(K,T)}{K^2}dK
\right].

$$

5. **Final Implied Variance Formula**: Combine the previous steps to obtain

$$
\begin{aligned}
E_t^{\mathbb Q}\!\left[\frac{1}{\tau}\int_t^T \sigma_s^2ds\right]
&=
\frac{2}{\tau}\ln\frac{F_{t,T}}{x}
-\frac{2}{\tau}\left(\frac{F_{t,T}}{x}-1\right)\\
&\quad+
\frac{2e^{r\tau}}{\tau}
\left[
\int_0^x\frac{P_t(K,T)}{K^2}dK
...
6. **VIX Special Case**: If you choose $x=F_{t,T}$, the correction terms vanish, and the fair variance swap strike becomes
...
\frac{2e^{r\tau}}{\tau}
\left[
\int_0^{F_{t,T}}\frac{P_t(K,T)}{K^2}dK
+\int_{F_{t,T}}^\infty\frac{C_t(K,T)}{K^2}dK
\right].

$$

This is the standard VIX / variance swap formula. VIX itself is the annualized square root of the 30-day implied variance, but the core derivation is the fair strike above.

**Takeaway**: 关键不是直接“预测波动率”，而是先用 options 静态复制 log contract，再用
...
把 realized variance 和 log payoff 连起来。对应 EF8083 slides 中 Hull Technical Note 22 的路线。

对应讲义可回看 [02_Implied_Volatility_VIX_VRP](Asset Pricing/Empirical AP/02_Implied_Volatility_VIX_VRP) 和 [Realized_Variance_Microstructure](Asset Pricing/Empirical AP/cards/Realized_Variance_Microstructure)。

題型四：文獻批判與實證分析（40分）

**Question:**  
Read and critique the empirical strategy of testing the **Fama-French Five-Factor Model $2015$**.

1. **Factor Zoo**: Economically justify why the Profitability $RMW$ and Investment $CMA$ factors were introduced into the pricing model.
2. **GRS Test**: State the null hypothesis of the **Gibbons-Ross-Shanken $1989$ test**. What does it mean for the intercept $\alpha _{i}$ if the model perfectly prices the test portfolios?
3. **P-hacking**: Discuss the concept of "Taming the Factor Zoo" as proposed by Feng, Giglio, and Xiu (2020, Journal of Finance). How does the selection of test portfolios affect the validity of asset pricing tests?

**Question.** Consider a representative-agent consumption-based asset pricing model. The stochastic discount factor is
...
r_{t+1}=\log R_{t+1},\qquad
\Delta c_{t+1}=\log C_{t+1}-\log C_t.

$$

Assume that $m_{t+1}$ and $r_{t+1}$ are conditionally jointly normal. Derive the conditional equity risk premium $\log E_t[R_{t+1}]-r_{f,t}$ and the real risk-free rate $r_{f,t}$. Then use the two expressions to explain the equity premium puzzle and the risk-free rate puzzle.

Finally, suppose the SDF is augmented by an additional risk factor $x_{t+1}$,

$$
m_{t+1}=\theta-\gamma\Delta c_{t+1}+b x_{t+1}.

$$

Explain how this additional factor can help resolve the two puzzles.

**标准 SDF 推导.** Euler equation 与 lognormal 条件给出：

$$
\begin{aligned}
E_t[M_{t+1}R_{t+1}]&=1,\\
m_{t+1}&=\log\beta-\gamma\Delta c_{t+1},\\
1&=E_t[e^{m_{t+1}+r_{t+1}}]\\
&=\exp\left(E_t[m_{t+1}]+E_t[r_{t+1}]
+\frac12 Var_t(m_{t+1}+r_{t+1})\right),\\
0&=E_t[m_{t+1}]+E_t[r_{t+1}]
+\frac12 Var_t(m_{t+1})
+\frac12 Var_t(r_{t+1})
+Cov_t(m_{t+1},r_{t+1}).
\end{aligned}
...
$$
\begin{aligned}
\log E_t[R_{t+1}]&=\log E_t[e^{r_{t+1}}]
=E_t[r_{t+1}]+\frac12 Var_t(r_{t+1}),\\
r_{f,t}&=-\log E_t[M_{t+1}]
=-E_t[m_{t+1}]-\frac12 Var_t(m_{t+1}).
\end{aligned}

$$

因此 conditional equity risk premium 为

$$
\begin{aligned}
\log E_t[R_{t+1}]-r_{f,t}
&=E_t[r_{t+1}]+\frac12 Var_t(r_{t+1})
+E_t[m_{t+1}]+\frac12 Var_t(m_{t+1})\\
&=-Cov_t(m_{t+1},r_{t+1})\\
&=\gamma Cov_t(\Delta c_{t+1},r_{t+1}).
\end{aligned}

$$

real risk-free rate 为

$$
\begin{aligned}
r_{f,t}
&=-E_t[m_{t+1}]-\frac12 Var_t(m_{t+1})\\
&=-\log\beta+\gamma E_t\Delta c_{t+1}
-\frac12\gamma^2 Var_t(\Delta c_{t+1})\\
&=\delta+\gamma E_t\Delta c_{t+1}
-\frac12\gamma^2 Var_t(\Delta c_{t+1}),
\qquad \delta\equiv-\log\beta.
\end{aligned}
...
**Equity premium puzzle.** 因为

$$
\log E[R]-r_f=\gamma Cov(\Delta c,r)
\le \gamma\sigma(\Delta c)\sigma(r),

$$

消费增长很平滑时，标准 CCAPM 只能靠很大的 $\gamma$ 生成足够高的 equity premium。例如

$$
\gamma\gtrsim\frac{0.06}{0.026\times0.20}\approx 11.5,

$$

若 $Corr(\Delta c,r)<1$，需要的 $\gamma$ 更高。核心问题是：consumption growth 太平滑，SDF 不够 volatile，无法匹配高 Sharpe ratio / high equity premium。

**Risk-free rate puzzle.** 但同一个 $\gamma$ 又进入

$$
r_f=\delta+\gamma E\Delta c-\frac12\gamma^2 Var(\Delta c).

$$

为了匹配 equity premium 而提高 $\gamma$ 后，$\gamma E\Delta c$ 往往把 $r_f$ 推高；同时 $-\frac12\gamma^2 Var(\Delta c)$ 使 $r_f$ 对消费风险过度敏感。结论是：

$$
\boxed{
\text{标准 CCAPM 很难同时解释 high equity premium 和 low stable risk-free rate。}
}
...
由通式 $\log E_t[R_{t+1}]-r_{f,t}=-Cov_t(m_{t+1},r_{t+1})$，

$$
\boxed{
\begin{aligned}
\log E_t[R_{t+1}]-r_{f,t}
&=-Cov_t(\theta-\gamma\Delta c_{t+1}+b x_{t+1},r_{t+1})\\
&=\gamma Cov_t(\Delta c_{t+1},r_{t+1})
-b Cov_t(x_{t+1},r_{t+1}).
\end{aligned}
}

$$

如果 $x_{t+1}$ 是 bad-state factor，坏状态中 $x_{t+1}$ 高、股票回报 $r_{t+1}$ 低，则 $Cov_t(x_{t+1},r_{t+1})<0$。当 $b>0$ 时，

$$
-b Cov_t(x_{t+1},r_{t+1})>0,

$$

因此 SDF 与股票回报更负相关，equity premium 上升，不再完全依赖 $\gamma Cov(\Delta c,r)$。
...
r_{f,t}
&=-E_t[m_{t+1}]-\frac12 Var_t(m_{t+1})\\
&=-\theta+\gamma E_t\Delta c_{t+1}-bE_t x_{t+1}\\
&\quad-\frac12\left[
\gamma^2 Var_t(\Delta c_{t+1})
+b^2 Var_t(x_{t+1})
-2\gamma b Cov_t(\Delta c_{t+1},x_{t+1})
\right].
\end{aligned}
}
...
关键是额外因子提高 $Var_t(m_{t+1})$，而

$$
r_{f,t}=-E_t[m_{t+1}]-\frac12 Var_t(m_{t+1}),

$$

所以更 volatile 的 SDF 会压低 risk-free rate。经济直觉：bad-state risk 让 safe asset 更像保险，投资者愿意付更高价格持有它，因此 risk-free return 更低。

$$
\boxed{
\text{bad-state factor }x
\Rightarrow
\begin{cases}
-Cov(m,r)\uparrow & \text{equity premium 上升},\\
Var(m)\uparrow & r_f\text{ 下降}.
\end{cases}
}
